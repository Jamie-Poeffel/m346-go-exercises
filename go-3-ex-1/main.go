...
func outputWithZodiacSign(p Person) {
	...
	if (p.Month == 4 && p.Day >= 20) || (p.Month == 5 && p.Day <= 20) {
		zodiacSign = Taurus
		...
		zodiacSign = Libra
	}else if (p.Month == 10 && p.Day >= 24) || (p.Month == 11 && p.Day <= 21)  {
		zodiacSign = Scorpius
...
func main() {
	...
	kush := Person{FullName{"Kush", "Srigiri"}, BirthDate{29, 6, 2007}}
	jamie := Person{FullName{"Jamie", "Poeffel"}, BirthDate{03, 10, 2007}}
	luis := Person{FullName{"Luis", "Allamand"}, BirthDate{19, 6, 2007}}
	...
	outputWithZodiacSign(kush)
	outputWithZodiacSign(jamie)
	outputWithZodiacSign(luis)
}
